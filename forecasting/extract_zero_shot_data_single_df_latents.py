...
class ExtractLatents:
    ...
    def process_data(self, loader, vqvae_model):
        ...
        # Initialize latent storage dictionaries
        x_latents_all = {}
        y_latents_all = {}
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(loader):
            ...
            x_codes, x_code_ids, codebook, x_latents = time2codes_with_latents(
                x_in_revin_space.permute(0, 2, 1), self.args.compression_factor, 
                vqvae_model.encoder, vqvae_model.vq
            )
            y_codes, y_code_ids, codebook, y_latents = time2codes_with_latents(
                y_in_revin_space.permute(0, 2, 1), self.args.compression_factor, 
                vqvae_model.encoder, vqvae_model.vq
            )
            ...
            # Store latents for first batch to initialize dictionaries
            if i == 0:
                for layer_name in x_latents:
                    x_latents_all[layer_name] = []
                    y_latents_all[layer_name] = []

            # Store latents for this batch
            for layer_name in x_latents:
                x_latents_all[layer_name].append(np.array(x_latents[layer_name].detach().cpu()))
                y_latents_all[layer_name].append(np.array(y_latents[layer_name].detach().cpu()))

            # expects code to be dim [bs x nvars x compressed_time]
            x_predictions_revin_space, x_predictions_original_space, x_decoder_latents = codes2time_with_latents(
                x_code_ids, codebook, self.args.compression_factor, 
                vqvae_model.decoder, self.revin_layer_x
            )
            y_predictions_revin_space, y_predictions_original_space, y_decoder_latents = codes2time_with_latents(
                y_code_ids, codebook, self.args.compression_factor, 
            ...
            # Store decoder latents for first batch to initialize dictionaries
            if i == 0:
                for layer_name in x_decoder_latents:
                    x_latents_all[f"decoder_{layer_name}"] = []
                    y_latents_all[f"decoder_{layer_name}"] = []

            # Store decoder latents for this batch
            for layer_name in x_decoder_latents:
                x_latents_all[f"decoder_{layer_name}"].append(np.array(x_decoder_latents[layer_name].detach().cpu()))
                y_latents_all[f"decoder_{layer_name}"].append(np.array(y_decoder_latents[layer_name].detach().cpu()))
        ...
        x_latents_concat = {}
        y_latents_concat = {}
        for layer_name in x_latents_all:
            x_latents_concat[layer_name] = np.concatenate(x_latents_all[layer_name], axis=0)
            y_latents_concat[layer_name] = np.concatenate(y_latents_all[layer_name], axis=0)
        ...
        # Add latents to data dictionary
        data_dict['x_latents'] = x_latents_concat
        data_dict['y_latents'] = y_latents_concat
        ...
        print("\nLatent shapes:")
        for layer_name, latent in data_dict['x_latents'].items():
            print(f"X {layer_name}: {latent.shape}")
        for layer_name, latent in data_dict['y_latents'].items():
            print(f"Y {layer_name}: {latent.shape}")
    ...
    def extract_data(self):
        ...
        latents_path = os.path.join(self.args.save_path, 'latents')
        if not os.path.exists(latents_path):
            os.makedirs(latents_path)

        print('Processing all data through VQVAE with latent extraction...')
...
def save_files(path, data_dict):
    """Save all processed data to files including latents"""
    # Save main data
    np.save(os.path.join(path, 'x_original.npy'), data_dict['x_original_arr'])
    np.save(os.path.join(path, 'y_original.npy'), data_dict['y_original_arr'])
    np.save(os.path.join(path, 'x_codes.npy'), data_dict['x_code_ids_all_arr'])
    np.save(os.path.join(path, 'y_codes.npy'), data_dict['y_code_ids_all_arr'])
    np.save(os.path.join(path, 'x_reverted.npy'), data_dict['x_reverted_all_arr'])
    np.save(os.path.join(path, 'y_reverted.npy'), data_dict['y_reverted_all_arr'])
    np.save(os.path.join(path, 'codebook.npy'), data_dict['codebook'])
    
    # Save latents
    latents_path = os.path.join(path, 'latents')
    
    print(f"\nSaving latents to {latents_path}/")
    for layer_name, latent in data_dict['x_latents'].items():
        save_path = os.path.join(latents_path, f'x_{layer_name}.npy')
        np.save(save_path, latent)
        print(f"  Saved x_{layer_name}.npy with shape {latent.shape}")
    
    for layer_name, latent in data_dict['y_latents'].items():
        save_path = os.path.join(latents_path, f'y_{layer_name}.npy')
        np.save(save_path, latent)
        print(f"  Saved y_{layer_name}.npy with shape {latent.shape}")


def time2codes_with_latents(revin_data, compression_factor, vqvae_encoder, vqvae_quantizer):
    '''
    Args:
    ...
    Returns:
        ...
        latents: dict of intermediate representations
    '''
    ...
    with torch.no_grad():
        ...
        latent, encoder_latents = encode_with_latents(flat_revin.to(torch.float), compression_factor, vqvae_encoder)
        
        # Store encoder latents
        for layer_name, layer_output in encoder_latents.items():
            # Reshape back to [bs, nvars, ...] format
            if len(layer_output.shape) == 3:  # Conv outputs [bs*nvars, channels, time]
                latents[layer_name] = layer_output.reshape(bs, nvar, layer_output.shape[1], layer_output.shape[2])
            elif len(layer_output.shape) == 2:  # Flattened outputs [bs*nvars, features]
                latents[layer_name] = layer_output.reshape(bs, nvar, layer_output.shape[1])
        
        # Vector quantization
        vq_loss, quantized, perplexity, embedding_weight, encoding_indices, encodings = vqvae_quantizer(latent)
        
        # Store pre and post quantization latents
        latents['pre_quantization'] = latent.reshape(bs, nvar, latent.shape[1], latent.shape[2])
        latents['post_quantization'] = quantized.reshape(bs, nvar, quantized.shape[1], quantized.shape[2])
    ...
    return codes, code_ids, embedding_weight, latents


def codes2time_with_latents(code_ids, codebook, compression_factor, vqvae_decoder, revin_layer):
    '''
    Args:
        code_ids: [bs x nvars x compressed_pred_len]
        codebook: [num_code_words, code_dim]
        compression_factor: int
        vqvae_decoder: trained vqvae decoder
    ...
    Returns:
        ...
        decoder_latents: dict of decoder intermediate representations
    '''
    bs = code_ids.shape[0]
    ...
    with torch.no_grad():
        ...
        # Decode with latent extraction
        prediction_recon, decoder_latents = decode_with_latents(quantized_swaped.to(device), compression_factor, vqvae_decoder)
        
        # Reshape decoder latents
        for layer_name, layer_output in decoder_latents.items():
            if len(layer_output.shape) == 3:  # Conv outputs [bs*nvars, channels, time]
                decoder_latents[layer_name] = layer_output.reshape(bs, nvars, layer_output.shape[1], layer_output.shape[2])
            elif len(layer_output.shape) == 2:  # Flattened outputs [bs*nvars, features]
                decoder_latents[layer_name] = layer_output.reshape(bs, nvars, layer_output.shape[1])
    ...
    return predictions_revin_space, predictions_original_space, decoder_latents


def encode_with_latents(inputs, compression_factor, encoder):
    """
    Encode inputs while capturing intermediate layer outputs
    """
    latents = {}
    ...
    if compression_factor == 4:
        x = encoder._conv_1(x)
        latents['conv_1'] = x.clone()
        x = torch.nn.functional.relu(x)

        x = encoder._conv_2(x)
        latents['conv_2'] = x.clone()
        x = torch.nn.functional.relu(x)

        x = encoder._conv_3(x)
        latents['conv_3'] = x.clone()
        x = encoder._residual_stack(x)
        latents['residual_stack'] = x.clone()
        x = encoder._pre_vq_conv(x)
        latents['pre_vq_conv'] = x.clone()
...
def decode_with_latents(inputs, compression_factor, decoder):
    """
    Decode inputs while capturing intermediate layer outputs
    """
    latents = {}
    
    if compression_factor == 4:
        x = decoder._conv_1(inputs)
        latents['conv_1'] = x.clone()

        x = decoder._residual_stack(x)
        latents['residual_stack'] = x.clone()

        x = decoder._conv_trans_1(x)
        latents['conv_trans_1'] = x.clone()
        x = torch.nn.functional.relu(x)

        x = decoder._conv_trans_2(x)
        latents['conv_trans_2'] = x.clone()
        x = torch.squeeze(x)
        ...
        x = decoder._conv_trans_A(x)
        latents['conv_trans_A'] = x.clone()
        x = torch.nn.functional.relu(x)
        ...
        x = decoder._conv_trans_3(x)
        latents['conv_trans_3'] = x.clone()
        x = torch.nn.functional.relu(x)

        x = decoder._conv_trans_4(x)
        latents['conv_trans_4'] = x.clone()
        x = torch.squeeze(x)
        ...
        x = decoder._conv_trans_B(x)
        latents['conv_trans_B'] = x.clone()
        x = torch.nn.functional.relu(x)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract VQVAE codes and latents for zero-shot forecasting')
